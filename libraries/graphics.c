...
Image* loadImage(const char* filename) {
    ...
    if ((fp = fopen(filename, "rb")) == NULL) { free(image); return NULL; }

    png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
    if (png_ptr == NULL) { free(image); fclose(fp); return NULL; }
    png_set_error_fn(png_ptr, NULL, NULL, user_warning_fn);
    info_ptr = png_create_info_struct(png_ptr);
    if (info_ptr == NULL) { free(image); fclose(fp); png_destroy_read_struct(&png_ptr, NULL, NULL); return NULL; }

    png_init_io(png_ptr, fp);
    png_set_sig_bytes(png_ptr, sig_read);
    png_read_info(png_ptr, info_ptr);
    png_get_IHDR(png_ptr, info_ptr, &width, &height, &bit_depth, &color_type, &interlace_type, NULL, NULL);
    if (width > 512 || height > 512) { free(image); fclose(fp); png_destroy_read_struct(&png_ptr, &info_ptr, NULL); return NULL; }
    ...
    if (!image->data) { free(image); fclose(fp); png_destroy_read_struct(&png_ptr, &info_ptr, NULL); return NULL; }

    line = (u32*) malloc(width * 4);
    if (!line) { free(image->data); free(image); fclose(fp); png_destroy_read_struct(&png_ptr, &info_ptr, NULL); return NULL; }
...
void blitImageToScreen(int sx, int sy, int width, int height, Image* source, int dx, int dy) {
    ...
    sceGuCopyImage(GU_PSM_8888, sx, sy, width, height, source->textureWidth,
                   source->data, dx, dy, PSP_LINE_SIZE, getVramDrawBuffer());
}
...
void blitAlphaImageToScreen(int sx, int sy, int width, int height, Image* source, int dx, int dy) {
    ...
    sceGuTexImage(0, source->textureWidth, source->textureHeight,
                  source->textureWidth, (void*) source->data);
...
/* … (unchanged Image creation, clear, fill, line draw, text, saveImage) … */
/* … keep your previous implementations here, only the fixes above matter … */
...
void flipScreen() {
    ...
    sceGuSwapBuffers();
    dispBufferNumber ^= 1;
    sceGuStart(GU_DIRECT, list); // start fresh for next frame
}
